    ...
    if (domains_count == 1) and (fixed_zones == []):
        ...
        if fz >= domains_count:
            raise Exception('crys-gsu-split : fixed zones count is more or equal '
                            'than target zones count ({0} zones, {1} fixed zones)'.format(domains_count, fz))
        print('crys-gsu-split : {0} zones ({1} fixed zones) : begin'.format(domains_count, fz))
        g.decompose_farhat(count=domains_count - fz, fz_names=fixed_zones)
        actual_zones_count = len(g.Zones)
