...
#---------------------------------------------------------------------------------------------------
    ...
    def ExtractNewZone(self, from_zone_index, from_face_index, faces_count, new_zone_name):
        """
        Извлечение из некоторой зоны подмножества ячеек для добавления в новую зону.

        Arguments:
            from_zone_index -- индекс зоны, из которой извлекаются ячейки,
            from_face_index -- индекс первой ячейки,
            faces_count -- количество ячеек,
            new_zone_name -- имя новой зоны.
        """

        # Получаем зоны.
        oz = self.Zones[from_zone_index]
        nz = Zone(new_zone_name)
        self.Zones.append(nz)

        # Разделяем ячейки.
        fs = oz.Faces
        ofs = fs[ : from_face_index] + fs[from_face_index + faces_count : ]
        nfs = fs[from_face_index : from_face_index + faces_count]
        oz.Faces = ofs
        nz.Faces = nfs
        ...
        oz.Nodes = []
        for n in ns:
            oz.Nodes.append(n.Copy())
        for i in range(count):
            oz.ReplaceNode(ns[i], oz.Nodes[i])
        oz.DeleteExtraNodes()
        oz.RestoreNodesIndices()
        #
        count = len(ns)
        nz.Nodes = []
        for n in ns:
            nz.Nodes.append(n.Copy())
        for i in range(count):
            nz.ReplaceNode(ns[i], nz.Nodes[i])
        nz.DeleteExtraNodes()
        nz.RestoreNodesIndices()
...
if __name__ == '__main__':
    g = Grid()
    g.Import('wing_1.dat')
    g.ExtractNewZone(0, 170, 10, 'Zone 2')
    g.Export('wing_1_mz.dat')
